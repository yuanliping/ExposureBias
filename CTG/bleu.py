...
class Scorer(object):
    ...
    def add(self, ref, pred):
        ...
        for w in ref_words + pred_words:
            if w not in self.dict:
                self.dict[w] = len(self.dict)
        ref = torch.IntTensor(list(map(lambda x: self.dict[x], ref_words)))
        pred = torch.IntTensor(list(map(lambda x: self.dict[x], pred_words)))
